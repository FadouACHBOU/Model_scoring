...
def filter_near_customer(df, cust_id, n_near_cust, target):
    ''' Function to filter dataframe finding the nearest neighbors of our customer in terms of probability.
...
st.sidebar.header('Choisir un dossier de prêt :')
...
client_pred_score = st.sidebar.checkbox('Résultat de la demande')
### Add checkbox for displaying score interpretation ###
score_interpret = st.sidebar.checkbox('Interprétation du score')
### Add checkbox for displaying client data analysis ###
client_analysis = st.sidebar.checkbox('Analyse des features ')


# ------------ Main display -----------------
## Title ##
st.title('Projet 7 Implémentez un modèle de scoring')
st.subheader('Fadia ACHIR BOULFELFEL')
...
st.write('### Informations générales clients :')
selections = st.multiselect('Veuillez sélectionner / désélectionner les données de la liste suivante :', df_test_sample_origin.columns.tolist(),
...
## Display loan answer regarding model probability calcul ##
if client_pred_score:
    st.write('### Décision : ')
    ### careful the url of the API should be change for serial deployment!! ###
    url_api_model_result = 'http://api-home-risk-p-07.herokuapp.com/scores'
    ...
    with st.expander('Détailles : '):
        st.write(""" Le résultat de l'API est un score compris entre 0 et 100% qui représente la probabilité de refus de prêt.  \n"""
            """ 1) Si le score est inférieur à 49% :  la demande de prêt est acceptée.  \n """
            """ 2) Si le score est entre 49 et 52% : la demande de prêt est refusée (mais peut être exeptionnellement acceptée selon appréciation du conseiller). \n"""
            """ 3) Si le  score est supérieur à 52% : la demande de prêt est refusée. """)
...
if score_interpret:
    ...
    st.write('#### Importance global des features (prédiction du modèle) ')
    selected_global_shap = st.selectbox("Veuillez sélectionner un graphique : ",
    ...
    st.write('#### Importance local des features (Prédiction du modèle pour le client ) ')
